(function(){
  ...
  var ACCEPT_EXT = { docx:1, docm:1, dotx:1, dotm:1, doc:1, dot:1, rtf:1, odt:1 };
  ...
  // ---------- View orchestration ----------
  var OOXML = { docx:1, docm:1, dotx:1, dotm:1 };
  ...
  function show(data, name){
    ...
    var ext = extOf(name);
    if (OOXML[ext]){
      var loading = el("div", "docx-msg"); loading.textContent = "Rendering document…"; dataView.appendChild(loading);
      window.docx.renderAsync(data, dataView, null, {
        className: "docx", inWrapper: true, breakPages: true, ignoreLastRenderedPageBreak: true,
        experimental: true, useBase64URL: true, renderHeaders: true, renderFooters: true
      }).then(function(){ if (loading.parentNode) loading.parentNode.removeChild(loading); })
        .catch(function(e){ legacyNotice(ext, e && e.message || e); });
    } else {
      legacyNotice(ext, null);
    }
    revealHeader();
  }
  function legacyNotice(ext, err){
    dataView.innerHTML = "";
    var box = el("div", "docx-notice");
    var h = el("div", "docx-notice-title"); h.textContent = "Can’t preview a “." + ext + "” file";
    var p = el("p");
    p.textContent = err
      ? ("This file couldn’t be rendered — it may be corrupt or password-protected. (" + err + ")")
      : ("This viewer renders modern Word documents (.docx, .docm, .dotx, .dotm). “." + ext + "” is an older binary or alternate format with no reliable in-browser renderer — open it in Word or LibreOffice and use “Save As → .docx” to view it here.");
  ...
  function readFile(file){
    if (!file) return;
    if (!isAccepted(file.name)){ if (!familyRoute(file)) toast("“" + file.name + "” isn’t a supported document type"); return; }
  ...
  // ---------- Family router (§6.10): wrong-file offer card + in-browser hand-off ----------
  ...
  // Sender — the routeGo click is a real user gesture, so no popup blocker; the
  // window handle is the message channel (no noopener on this one window.open).
  ...
  // Receiver — a sibling family tab hands the File across via postMessage (§6.10).
  ...
  if (fvh){
    ...
    if (window.opener){
      ...
      if (emptySub){
        ...
        var receivingMsg = "Receiving “⁨" + fvhName + "⁩”…";
        emptySub.textContent = receivingMsg;                  // textContent only — untrusted name
        setTimeout(function(){ if (emptySub.textContent === receivingMsg) emptySub.textContent = emptySubPrev; }, 10000);
      }
    }
  }
